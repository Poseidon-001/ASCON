...
void ascon_encrypt(uint8_t *ciphertext, const uint8_t *key, const uint8_t *nonce, const uint8_t *associateddata, size_t adlen, const uint8_t *plaintext, size_t ptlen, const char *variant) {
    ...
    if (adlen > 0) {
        ...
        uint8_t *a_padding = (uint8_t *)malloc(a_padding_len + 1);
        if (a_padding == NULL) {
        ...
        uint8_t *a_padded = (uint8_t *)malloc(adlen + a_padding_len + 1);
        if (a_padded == NULL) {
            free(a_padding);
            fprintf(stderr, "Memory allocation failed\n");
            return;
    ...
    uint8_t *p_padding = (uint8_t *)malloc(p_padding_len + 1);
    if (p_padding == NULL) {
        fprintf(stderr, "Memory allocation failed\n");
        return;
    }
    ...
    uint8_t *p_padded = (uint8_t *)malloc(ptlen + p_padding_len + 1);
    if (p_padded == NULL) {
        free(p_padding);
        fprintf(stderr, "Memory allocation failed\n");
        return;
...
void ascon_decrypt(uint8_t *plaintext, const uint8_t *key, const uint8_t *nonce, const uint8_t *associateddata, size_t adlen, const uint8_t *ciphertext, size_t ctlen, const char *variant) {
    ...
    uint8_t *c_padding = (uint8_t *)malloc(c_padding_len + 1);
    if (c_padding == NULL) {
        fprintf(stderr, "Memory allocation failed\n");
        return;
    }
    ...
    uint8_t *c_padded = (uint8_t *)malloc(ctlen - taglen + c_padding_len + 1);
    if (c_padded == NULL) {
        free(c_padding);
        fprintf(stderr, "Memory allocation failed\n");
        return;
...
void ascon_process_ciphertext(uint64_t *S, uint8_t *plaintext, const uint8_t *ciphertext, size_t ctlen, int b, int rate) {
    ...
    uint8_t *c_padded = (uint8_t *)malloc(ctlen + c_padding_len + 1);
    if (c_padded == NULL) {
        free(c_padding);
        fprintf(stderr, "Memory allocation failed\n");
        return;
...
void demo_aead(const char *variant) {
    ...
    if (receivedplaintext == NULL) {
        fprintf(stderr, "Memory allocation failed for receivedplaintext\n");
        free(plaintext);
        free(ciphertext);
        exit(EXIT_FAILURE);
    }
    ...
    FILE *output_file = fopen("ascon2.txt", "w");
    if (!output_file) {
    ...
    double encryption_time = ((double)(end_encryption - start_encryption)) / CLOCKS_PER_SEC * 1000.0;
    ...
    double decryption_time = ((double)(end_decryption - start_decryption)) / CLOCKS_PER_SEC * 1000.0;
